...
const SingleGroupChatItem = ({activity}) => {
    ...
    const handleActivityChat = ()=>{
        history.push(`/chats/${activity._id}`);
    };
        ...
        <Wrapper onClick = {()=> handleActivityChat()} >
            <Container >
                <Title>
                    ...
                    <div>
                        On {moment(activity.activityDate.date, 'YYYY-MM-DD').format('ddd MMMM Do YYYY')}
                    </div>
                </Title>
                <Details>
                    <Time>
                        <FiCalendar/>
                        <span>Time: {moment(activity.activityDate.from , 'HH:mm').format('hh:mm A')} - {moment(activity.activityDate.to , 'HH:mm').format('hh:mm A')}</span>
                    </Time>
                    <Members>
                        <FiUsers/>
                        <span>{activity.joining.length} poeple joined this activity</span>
                    </Members>
                </Details>
                <Address>
                    <FiMapPin/>
